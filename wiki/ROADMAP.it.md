...
`wiki_context.py` esegue una ricerca vettoriale e restituisce le pagine rilevanti come blocco `<wiki-context>`. In questa repository viene usato dal backend embedded di `master-dnd-plugin/wiki-backend`; il plugin OpenClaw standalone vive in una repository separata.
